function createTree(newickData) {
    ...
    //     d3.select(element).select("circle").style("fill", "steelblue");
    // });
    // Set the layout to "radial".
    //tree.layout = "radial";

    // Adjust the layout
    //tree(d3.layout.newick_parser(newickData))
    tree(example_tree)
        .layout();
}

function createTreeSimple(newickData) {
    var tree = d3.layout.phylotree()
      .svg(d3.select("#tree-container"));
    tree(d3.layout.newick_parser(newickData));
}